...
function popup(p){let c=firstPhone(p.call,p.mo,p.doctor,p.pilot),chips=W.services(p).map(x=>`<span class="pchip">${W.esc(x)}</span>`).join(''),cls=W.cls(p);let d=[];if(p.place&&!(p.label||'').includes(p.place))d.push('📍 '+W.esc(p.place));if(p.vehicle)d.push('🚗 '+W.esc(p.vehicle));if(p.base)d.push('🏷 '+W.esc(p.base));if(p.date)d.push('📅 '+W.esc(p.date)+(p.day?' · '+W.esc(p.day):''));let contacts=popContactRows(p);let hk=p.photo?`<img class="pop-thumb" src="${(typeof p.photo==='string'&&p.photo.indexOf('/')>-1)?p.photo:HK_PHOTO}" loading="lazy" onerror="this.remove()"/>`:'';return `<div class="pop pop-${cls}"><div class="phead"><span class="picon">${W.icon(p)}</span><div><div class="ptitle">${W.esc(p.label)}</div><div class="psub">${W.esc(W.NAMES[p.palkhi]||p.palkhi)}</div></div></div><div class="pbody"><div class="pchips">${chips}</div>${p.live?liveBlock(p.live):''}${hk}${d.length?`<div class="pdet">${d.join('<br>')}</div>`:''}${contacts}<div class="pact">${c?`<a class="pbtn call" href="tel:${phoneHref(c)}">📞 फोन करा</a>`:''}<a class="pbtn dirbtn" target="_blank" href="${dir(p)}">🧭 दिशा</a></div></div></div>`}
