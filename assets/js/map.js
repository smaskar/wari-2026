...
function popup(p){let c=W.tel(p.call),chips=W.services(p).map(x=>`<span class="pchip">${W.esc(x)}</span>`).join('');let d=[];if(p.place&&!(p.label||'').includes(p.place))d.push(W.esc(p.place));if(p.vehicle)d.push('🚗 '+W.esc(p.vehicle));if(p.mo)d.push('🧑‍⚕️ '+W.esc(p.mo));if(p.doctor)d.push(W.esc(p.doctor));if(p.pilot)d.push(W.esc(p.pilot));if(p.base)d.push(W.esc(p.base));if(p.date)d.push('📅 '+W.esc(p.date));let hk=p.photo?`<img class="pop-thumb" src="${(typeof p.photo==='string'&&p.photo.indexOf('/')>-1)?p.photo:HK_PHOTO}" loading="lazy" onerror="this.remove()"/>`:'';return `<div class="pop"><div class="ptitle">${W.icon(p)} ${W.esc(p.label)}</div><div class="psub">${W.esc(W.NAMES[p.palkhi]||p.palkhi)}</div><div class="pchips">${chips}</div>${p.live?liveBlock(p.live):''}${hk}${d.length?`<div class="pdet">${d.join('<br>')}</div>`:''}<div class="pact">${c?`<a class="pbtn call" href="tel:${c}">📞 फोन</a>`:''}<a class="pbtn" target="_blank" href="${dir(p)}">🧭 दिशा</a></div></div>`}
