...
function renderClips(){let el=$('clipStrip'),clips=window.WARI_CLIPS||[];if(!el||!clips.length)return;el.innerHTML=clips.map((c,i)=>`<button type="button" class="clip-card" onclick="openClip(${i})"><span><span class="clip-play">▶</span><span class="clip-title">${W.esc(c.title)}</span></span><span class="clip-meta">व्हिडिओ · ${W.esc(c.size||'')}</span></button>`).join('')}
