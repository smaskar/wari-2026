...
function renderClips(){let el=$('clipStrip'),clips=window.WARI_CLIPS||[];if(!el||!clips.length)return;
el.innerHTML=clips.map((c,i)=>`<details class="clip-item"${i===0?' open':''}><summary class="clip-sum"><span class="clip-ic">▶</span><span class="clip-tt"><b>${W.esc(c.title)}</b><small>${W.esc(c.size||'')}</small></span><span class="clip-chev">⌄</span></summary><div class="clip-body"><iframe class="clip-frame" data-src="${W.escAttr(clipPreviewUrl(c))}"${i===0?` src="${W.escAttr(clipPreviewUrl(c))}"`:''} allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe><a class="clip-drive" href="${W.escAttr(clipViewUrl(c))}" target="_blank" rel="noopener noreferrer">Google Drive मध्ये उघडा ↗</a></div></details>`).join('');
